...
import { mockForexVendors } from './mockForexData';
...
export default function ForexCanvas({ onClose }: ForexCanvasProps) {
  ...
  const [results, setResults] = useState(mockForexVendors);
  const [selectedTags, setSelectedTags] = useState<string[]>(['Best Rate', 'Home Delivery']);
  ...
  const convertedAmount = (amount / 82.5).toFixed(2);
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-slate-900">${convertedAmount}</span>
                  </div>
                  <p className="mt-1 text-xs text-slate-500">{fromCurrency} → {toCurrency} • Live Rate: 82.50</p>
                </div>
        ...
        <div className="p-4">
          {loading ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-slate-200 bg-slate-50 p-8">
              <div className="mb-3 h-10 w-10 animate-spin rounded-full border-3 border-slate-200 border-t-cyan-600" />
              <p className="text-sm font-semibold text-slate-600">Finding vendors...</p>
            </div>
          ) : results.length > 0 ? (
            <div className="space-y-3">
              <p className="text-xs font-semibold text-slate-500">{results.length} vendors found</p>
                ...
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="text-xs text-slate-600">
                      ...
                      <p className="text-slate-500">{vendor.delivery}</p>
                    </div>
