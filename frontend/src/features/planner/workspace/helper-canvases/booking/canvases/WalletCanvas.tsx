    ...
    <div className="flex h-full w-full flex-col bg-paper-1 p-4 lg:p-6 select-none overflow-y-auto custom-scrollbar">
      ...
      <div className="flex items-center justify-between border-b border-line pb-3 mb-4">
        <div className="flex items-center gap-2">
          <Ticket size={18} className="text-ink-700" />
          <h3 className="text-lg font-bold uppercase tracking-wider text-ink-900">Travel Wallet</h3>
        </div>
        <button
          onClick={onClose}
          className="rounded-full p-1.5 text-ink-400 hover:bg-paper-1 hover:text-ink-700 transition-colors"
        >
          <X size={18} />
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-4">
        {passes.length === 0 ? (
          <div className="flex flex-col items-center justify-center text-center py-12 px-4 bg-paper-2 rounded-2xl border border-line">
            <Ticket size={28} className="mb-3 text-ink-400" />
            <h4 className="text-sm font-bold text-ink-800 uppercase tracking-widest">No Active Passes</h4>
              ...
              return (
                <div
                  ...
                  className={`relative rounded-2xl border overflow-hidden cursor-pointer shadow-xs transition-all duration-300 ${FOCUS_RING_CLASS} ${
                    isExpanded
                      ? 'border-cat-stay bg-paper-2 ring-2 ring-cat-stay/10'
                      : 'border-line bg-paper-2 hover:border-line-strong'
                  }`}
                >
                  {/* Top Bar: Ticket Theme Color depending on type */}
                  <div className={`h-1.5 w-full ${isHotel ? 'bg-cat-stay' : 'bg-blue-600'}`} />

                  <div className="p-4 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${isHotel ? 'bg-cat-stay/10 text-cat-stay' : 'bg-blue-50 text-blue-600'}`}>
                          {isHotel ? <Building size={16} /> : <Plane size={16} />}
                        </div>
                        <div>
                          <h4 className="text-xs font-extrabold text-ink-900 truncate max-w-[180px]">{pass.title}</h4>
                          <p className="text-[9px] font-bold text-ink-400 uppercase tracking-wider mt-0.5">{pass.subtitle}</p>
                        </div>
                      </div>
                      <div className="text-right shrink-0">
                        <span className="text-[9px] uppercase font-black text-ink-400 tracking-wider">Booking Ref</span>
                        <h5 className="text-xs font-black text-ink-900 mt-0.5">{pass.pnr}</h5>
                      </div>
                    </div>

                    {/* Details row */}
                    <div className="grid grid-cols-3 gap-2 border-t border-dashed border-line pt-3 text-left">
                      <div>
                        <span className="text-[8px] uppercase font-black text-ink-400 tracking-wider">Date / Day</span>
                        <p className="text-[10px] font-bold text-ink-700 mt-0.5 truncate">{pass.date}</p>
                      </div>
                      <div>
                        <span className="text-[8px] uppercase font-black text-ink-400 tracking-wider">{isHotel ? 'Check-In' : 'Gate / Platform'}</span>
                        <p className="text-[10px] font-bold text-ink-700 mt-0.5 truncate">{isHotel ? pass.checkIn : pass.gate}</p>
                      </div>
                      <div>
                        <span className="text-[8px] uppercase font-black text-ink-400 tracking-wider">{isHotel ? 'Room Class' : 'Seat Allocation'}</span>
                        <p className="text-[10px] font-bold text-ink-700 mt-0.5 truncate">{isHotel ? pass.room : pass.seat}</p>
                      </div>
                    </div>

                    {/* QR Code and check-in summary when clicked */}
                    {isExpanded && (
                      <div className="border-t border-line pt-4 mt-1 flex flex-col items-center gap-3 bg-paper-0/50 rounded-xl p-3 animate-slide-down">
                        {/* Barcode/QR Mockup */}
                        <div className="bg-paper-2 p-2.5 rounded-lg border border-line shadow-3xs flex flex-col items-center gap-1.5">
                          <QrCode className="text-ink-800" size={80} strokeWidth={1.5} />
                          <span className="text-[8px] font-bold tracking-widest text-ink-400 uppercase">{pass.pnr}-{pass.id.substring(0, 4).toUpperCase()}</span>
                        </div>
                        <p className="text-[10px] text-ink-400 font-semibold max-w-[200px] text-center">
