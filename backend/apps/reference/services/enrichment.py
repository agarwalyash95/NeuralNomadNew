...
apps.knowledge.services.enrichment — same content, PlaceInsight/LocalTip
imports now point at apps.reference.models (where the models themselves
moved to, same tables, see reference.migrations.0018_phase7_knowledge_migration).
apps.knowledge.services.enrichment re-exports this module's public names as
...
def _save_insight(instance, insight_type, *, text="", structured=None, tier="suggested", basis="", ttl_days=14):
    ...
    PlaceInsight.objects.update_or_create(
        ...
        defaults={
            ...
            "provenance": make_provenance(tier, source=DEFAULT_GEMINI_MODEL, basis=basis),
            "expires_at": timezone.now() + timedelta(days=ttl_days),
        },
    )


def _gemini_json(prompt, response_schema, *, model=None, temperature=0.2):
...
def run_safety_etiquette_pass(batch_size=5):
    ...
    from apps.reference.models import City, LocalTip
...
def run_enrichment_pass(batch_size_per_category=5):
    ...
    """
    from apps.reference.models import ActivityMaster, AttractionMaster, HotelMaster, PlaceInsight, RestaurantMaster
    ...
    for category, model in models_by_category.items():
        ...
        already_done_ids = set(
            PlaceInsight.objects.filter(content_type=content_type, insight_type=_PROXY_INSIGHT_TYPE[category])
